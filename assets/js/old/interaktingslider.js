...
InteraktingSlider.prototype.getCommande = function(){
	...
	html += "<div class='navi_slider'>";
	for (i=0;i<this.countSlides();i++){
		html += "<a href='#' id='button"+i+"' class='bullet' onclick='interaktingslider.goTo("+i+");return false;'><span>"+(i+1)+"</span></a>";		
	}
	html += "</div>";
...
InteraktingSlider.prototype.setCurrentButton = function(){
	for (i=0;i<this.countSlides();i++){
		...
		if(this.isCurrentSlide(i)){
			button.setAttribute("class", "active bullet"); 
			button.setAttribute("className", "active ");
		}
		else{
			button.setAttribute("class", "normal bullet"); 
